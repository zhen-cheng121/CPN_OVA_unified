...
from models.resnet_cpn_dce import *
from models.wideresnet_cpn_dce import *
...
parser = argparse.ArgumentParser(description='Evaluates a CIFAR OOD Detector and Misclassification Detector (DCE)',
...
parser.add_argument('--score', default='dist', type=str, choices=['dist', 'prob'], help='score options: dist|prob')

parser.add_argument('--model', default='resnet18', help='directory of model for saving checkpoint')
parser.add_argument('--model-path', default='.checkpoint/res18-', help='directory of model for saving checkpoint')
parser.add_argument('--temp', default=0.01, type=float, help='temperature of Euclidian distance')
...
if args.model == 'resnet18':
    net = ResNet18_cpn(num_classes=num_classes).cuda()
elif args.model == 'wrn-28-10':
    net = WideResNet_CPN(depth=28, num_classes=num_classes, widen_factor=10, dropRate=0.0).cuda()
elif args.model == 'wrn-40-2':
    net = WideResNet_CPN(depth=40, num_classes=num_classes, widen_factor=2, dropRate=0.0).cuda()
...
def get_ood_scores(loader, in_dist=False):
    ...
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(loader):
            ...
            elif args.score == 'prob': # original MSP and Mahalanobis (but Mahalanobis won't need this returned)
                _score.append(-np.max(smax, axis=1))
                list_softmax.extend(np.max(smax, axis=1))
