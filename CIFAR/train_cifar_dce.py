...
dataset_options = ['cifar-10', 'cifar-100', 'tiny-imagenet']
...
parser.add_argument('--temp', type=float, default=1.0, metavar='LR', help='temperature parameter')
parser.add_argument('--temp_warm_epoch', type=int, default=40, metavar='LR', help='temperature parameter')
parser.add_argument('--pl_weight', type=float, default=0.05, metavar='LR', help='temperature parameter')
...
def main():
    ...
    if args.model == 'resnet18':
        model = ResNet18_cpn(num_classes=num_classes).to(device)
    elif args.model == 'wrn-28-10':
        model = WideResNet_CPN(depth=28, num_classes=num_classes, widen_factor=10, dropRate=0.0).to(device)
    elif args.model == 'wrn-40-2':    
        model = WideResNet_CPN(depth=40, num_classes=num_classes, widen_factor=2, dropRate=0.0).to(device)        
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        temperature_scale = torch.min(torch.tensor((epoch / args.temp_warm_epoch * args.temp, args.temp)))   # temper warm-up          
        model.train()
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            loss = loss_ce + pl_loss
            loss.backward()
            
            optimizer.step()
            train_loss += loss_ce.item() * target.size(0)
