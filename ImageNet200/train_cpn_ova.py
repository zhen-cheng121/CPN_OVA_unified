...
parser.add_argument('--method', default='ova', type=str, help=' using OVA training')
...
parser.add_argument('--mix_ova', type=float, default=1.0, help='weight of ova loss in mixce')
...
def main():
    ...
    output_folder = root + '/' + args.arch + '_' + args.method + '_' + str(args.num_class) + '_' + str(args.temp) + '_' + str(args.pl_weight) + '_' + str(args.mix_ova)
...
def main_worker(local_rank, nprocs, args):
    ...
    model = Custom_ResNet(args.arch, classnum, args.temp, args.thres_type)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        lr_scheduler.step()
        print("epoch: ", epoch)
        model.module.temperature_scale = torch.min(torch.tensor(( (epoch + 1) / 30 * args.temp, args.temp)))
        # train for one epoch
        train(train_loader, train_loader_ood, model, criterion, optimizer, epoch, local_rank, args)
...
def train(train_loader, train_loader_ood, model, criterion, optimizer, epoch, local_rank, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        logits = -(dist - model.module.rejection_threshold) * model.module.temperature_scale
        ...
        loss_ova = criterion_ova(logits, one_hot) * args.num_class
        zeros = torch.zeros(logits.shape[0], 1).cuda()
        logits = torch.cat((logits, zeros), dim=1)
        loss_ce = criterion(logits, target)
        loss_cls = args.mix_ova * loss_ova + (1 - args.mix_ova) * loss_ce
...
def validate(epoch, val_loader, model, criterion, local_rank, args):
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            one_hot = torch.zeros(target.size(0), args.num_class).cuda().scatter_(1, target.view(-1, 1), 1)
            dist, features, centers = model(images)
            output = -(dist - model.module.rejection_threshold) * model.module.temperature_scale            
